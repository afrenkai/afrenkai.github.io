...
- tldr paper
- explain sde, what happened prior
- web demo
- provide de-slopped ipynb
- higher-scale (DiT) demo as well
- failure modes
...
class ChordEditPipeline(DiffusionPipeline):
    ...
    def prepare_image_tensor(self, image: Image.Image | torch.Tensor) -> torch.Tensor:
    ...
    def encode_image_to_latent(self, pixel_values: torch.Tensor) -> torch.Tensor:
    ...
    def apply_safety_checker(self, images: torch.Tensor) -> tuple[torch.Tensor, List[bool]]:
    ...
    def tensor_to_pil(self, tensor: torch.Tensor) -> List[Image.Image]:
        tensor = tensor.detach().cpu().clamp(0.0, 1.0)
        to_pil = transforms.ToPILImage()
        return [to_pil(sample) for sample in tensor]

    def build_vae_transform(self) -> transforms.Compose:
        ops: List[Any] = []
    ...
    def prepare_edit_params(self, cfg: Dict[str, Any]) -> Dict[str, Any]:
        params = dict(cfg)
    ...
    def time_to_index(self, batch: int, t_scalar: float, device, dtype=torch.long):
    ...
    def get_alpha_sigma(self, tensor: torch.Tensor, timesteps: torch.Tensor):
    ...
    def pred_x0(self, x_anchor, timesteps, cond, noise):
    ...
    def u_estimate(self, x_anchor, src_embed, edit_embed, noise, t_s: float, delta: float):
